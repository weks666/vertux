...
tutorial.accounts[0].portfolioHistory = values.map((value,index) => ({
  asOf: new Date(Date.UTC(2026,6,31+index,18)).toISOString(),
...
const earlier=[];
for(let time=Date.UTC(2025,6,31,18);time<Date.UTC(2026,6,31,18);time+=7*86400000){
  const progress=(time-Date.UTC(2025,6,31,18))/(365*86400000);
...
tutorial.accounts[0].portfolioHistory.sort((a,b)=>a.asOf.localeCompare(b.asOf));
...
for (const name of result.files) {
  ...
  if (name === 'index.html') {
    ...
    text = text.replace('</head>', '  <link rel="stylesheet" href="./demo-frame.css">\n</head>');
