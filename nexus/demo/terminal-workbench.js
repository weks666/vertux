...
const emptyLayout=()=>({tabs:[],active:'',count:1,watchCollapsed:false,sideCollapsed:false,dockCollapsed:false,dock:'stats',side:'market',linkPeriod:false,linkCursor:false,radar:{change:0,volume:0,distance:0},radarSaved:[]});
...
export function initTerminal({request,getSelection,getBootstrap,onSelection,onChart,showToast=()=>{}}){
 ...
 function marketPlacement(right){
  const book=$('#terminalBook'),tape=$('#terminalTape');if(!book||!tape)return;
  if(right){$('#terminalSideBook').append(book);$('#terminalSideTape').append(tape);book.hidden=false;tape.hidden=false;}
  else {dock.querySelector('.terminal-dock-body').append(book,tape);book.hidden=layout.dock!=='book';tape.hidden=layout.dock!=='tape';}
  side.querySelector('[data-market-side]').hidden=right;
  for(const el of side.querySelectorAll('#terminalSideMarket>header'))el.hidden=!right;
 }
 function sideTab(name,persist=true){
  layout.side=name;for(const b of side.querySelectorAll('[data-side]')){b.setAttribute('aria-selected',String(b.dataset.side===name));b.tabIndex=b.dataset.side===name?0:-1;}
  $('#terminalPlan').hidden=name!=='plan';$('#terminalSideAlerts').hidden=name!=='alerts';$('#terminalSideMarket').hidden=name!=='market';
  side.querySelector('#terminalNote').closest('label').hidden=name==='market';
  marketPlacement(name==='market'&&!['book','tape'].includes(layout.dock));if(persist)dirty('layout');
 }
 ...
 function layoutView(){panel.classList.toggle('watch-collapsed',layout.watchCollapsed);panel.classList.toggle('side-collapsed',layout.sideCollapsed);panel.classList.toggle('dock-collapsed',layout.dockCollapsed);
  ...
  main.querySelector('.terminal-grid').dataset.count=expandedId?1:layout.count;renderTabs();renderCharts();sideTab(layout.side||'market',false);}
 ...
 panel.addEventListener('click',event=>{const button=event.target.closest('button');if(!button)return;const data=button.dataset,id=button.closest('[data-chart-id]')?.dataset.chartId;
  ...
  else if(button.hasAttribute('data-market-bottom')){marketPlacement(false);dockTab('book');}
  else if(button.hasAttribute('data-market-side'))marketPlacement(true);
 ...
 let initial=init();
 document.addEventListener('keydown',e=>{if(e.key==='Escape'&&expandedId)expandChart(null);if(visible&&e.altKey&&e.key==='Enter'){e.preventDefault();expandChart(expandedId?null:layout.active,document.activeElement);}});
