...
export function createMarketChart(host,candles,{onCrosshair,onState,onMode,onRange,onHistory,onDrawingSelect,onReplay,getDrawingDefaults,onDrawingStyle}={}) {
 ...
 let colors={},sourceRows=rows,replay=null,replayTimer=null,replaySpeed=1;
 ...
 function getState(){return {type,scaleMode,inverted,colors:{...colors},magnet:drawing.isMagnet(),drawingsHidden:drawing.isHidden(),indicators:{...enabled},studies:studies.map(s=>({...s})),drawings:drawing.getState(),range:replay?replay.range:chart.timeScale().getVisibleRange(),flash};}
 ...
 return {chart,drawings:drawing,getState,getRows:()=>rows.slice(),getSeries:price,setData,
  ...
  destroy(){clearInterval(replayTimer);replayTimer=null;dead=true;cancelAnimationFrame(historyFrame);clearTimeout(pulseTimer);themeObserver.disconnect();resize.disconnect();drawing.destroy();chart.unsubscribeCrosshairMove(cross);chart.timeScale().unsubscribeVisibleLogicalRangeChange(rangeChanged);chart.remove();}};
}
