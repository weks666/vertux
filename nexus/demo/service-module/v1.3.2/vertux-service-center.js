(function () {
  ...
  class VertuxServiceCenter extends HTMLElement {
    ...
    async refreshQuietly() {
      if(document.hidden || !this.isConnected || this.closest('[hidden]') || this.state.busy || this.state.loading || this.refreshing || !this.state.data) return;
      ...
      try {
        ...
        if(JSON.stringify(next)!==JSON.stringify(this.state.data)||JSON.stringify(messages)!==JSON.stringify(this.state.messages)){
          this.state.data=next;this.state.messages=messages;this.render();
        }
      } catch { /* Keep the saved view and draft; the next refresh retries. */ }
    ...
    renderSubscription() {
      ...
      if (data.investPresentation || /^(?:kovrocity|vertux)-invest-workspace$/.test(data.currentProduct?.slug||'')) return this.renderPersonalInvest();
    ...
    renderInvestDevices() {
      const devices=Array.isArray(this.state.data.investDevices)?this.state.data.investDevices:[];
      return `<section class="panel invest-devices"><div class="panel-title"><h3>Ваши устройства</h3></div><p class="muted">Компьютеры с доступом к этому Invest</p>${devices.length?`<strong class="device-count">${devices.length}</strong><div class="rows">${devices.map(device=>`<div class="device-row">${icon('device')}<span><b>${escapeText(device.name||'Компьютер')}</b><small>${device.lastUsedAt?'Последний вход: '+dateText(device.lastUsedAt,true):'Зарегистрировано: '+dateText(device.registeredAt)}</small></span><span class="device-dot" aria-label="Доступ разрешён"></span></div>`).join('')}</div>`:'<p class="empty small">Зарегистрированных компьютеров пока нет.</p>'}<p class="privacy-note">Управление устройствами доступно в настройках Nexus.</p></section>`;
    }

    renderPersonalInvest() {
      const current=this.state.data.subscription;
      return `<div class="subscription-layout personal-invest-subscription"><section class="panel current-plan"><div class="panel-title"><h3>Текущий доступ</h3></div><div class="plan-identity"><span class="plan-symbol">${icon('crown')}</span><div><h3>Invest Workspace</h3><p><span class="status">${escapeText(statusLabel[current?.status]||'Условия доступа в Nexus')}</span>${current?.periodEnd?`<span class="plan-date">До ${dateText(current.periodEnd)}</span>`:''}</p></div></div><h4>Ваше пространство для инвестиций</h4>${this.renderPlanFeatures(['Портфель и операции','Аналитика и графики','События и уведомления','План и журнал идей'])}<div class="plan-ai-note"><strong>Доступ через ваш аккаунт Nexus</strong><p class="muted">Данные портфеля и ключи брокера остаются на вашем компьютере.</p></div></section>${this.renderInvestDevices()}<section class="panel subscription-history"><div class="panel-title"><h3>Продление и условия</h3></div><p>Для этого пространства действуют условия выданного доступа. Отдельная подписка Invest здесь ещё не оформлена.</p><button type="button" class="button" data-section="support">Обсудить подписку с поддержкой</button></section></div>`;
    }
    ...
    renderStandardSubscription(billing) {
      ...
      const start=Date.parse(current?.periodStart),end=Date.parse(current?.periodEnd),now=Date.now();
      const remaining=Number.isFinite(end)?Math.max(0,Math.ceil((end-now)/86400000)):null;
      const progress=Number.isFinite(start)&&Number.isFinite(end)&&end>start?Math.max(0,Math.min(100,(end-now)/(end-start)*100)):null;
      ...
      return `<div class="subscription-layout">
        <section class="panel current-plan" aria-label="Состояние подписки">
          ...
          <div class="plan-identity"><span class="plan-symbol" aria-hidden="true">${icon('crown')}</span><div><h3>Invest Workspace</h3><p><span class="status">${escapeText(statusLabel[current?.status] || (current ? current.status : 'Ещё не оформлена'))}</span>${current?.periodEnd ? `<span class="plan-date">Доступ до ${dateText(current.periodEnd)}</span>` : ''}</p></div></div>
          ${remaining!==null ? '<div class="plan-remaining"><span>До окончания периода</span><strong>'+remaining+' дн.</strong>'+(progress!==null?'<div class="plan-progress" role="progressbar" aria-label="Оставшаяся часть периода" aria-valuenow="'+Math.round(progress)+'" aria-valuemin="0" aria-valuemax="100"><i style="width:'+progress.toFixed(2)+'%"></i></div>':'')+'</div>':''}
          ...
          <div class="plan-ai-note"><strong>Личная подписка</strong><p class="muted">Доступ привязан к вашему аккаунту. Локальные данные и настройки сохраняются на вашем компьютере.</p></div>
        </section>
        ${this.renderInvestDevices()}
        <section class="panel billing-purchase"><div class="panel-title"><h3>Продлить подписку</h3></div>
          ...
          <fieldset class="billing-terms" ${this.state.busy || pending ? 'disabled' : ''}><legend>Срок подписки</legend>
            ${billing.catalog.terms.map(term => `<label class="billing-term"><input type="radio" name="billing-months" value="${term.months}" ${selected.months === term.months ? 'checked' : ''}>
              <span><b>${escapeText(term.label)}</b><strong>${money(term.amountMinor)}</strong><small>${money(Math.round(term.amountMinor / term.months / 100) * 100)} в месяц${term.months > 1 ? ` · экономия ${money(billing.catalog.monthlyPriceMinor * term.months - term.amountMinor)}` : ''}</small></span></label>`).join('')}
          </fieldset>
