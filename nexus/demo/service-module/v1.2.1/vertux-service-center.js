(function () {
  ...
  class VertuxServiceCenter extends HTMLElement {
    ...
    get bridge() {
      return window.nexusProduct && window.nexusProduct.service;
    }
    ...
    render() {
      ...
      this.contentRoot.setAttribute('aria-busy', this.state.loading ? 'true' : 'false');
      this.bind();
    }
      ...
      return `<div class="support-layout">
        ...
        <section class="panel compose">
          ...
          ${this.state.data.capabilities.createSupportTicket ? `<form class="form" data-form="ticket">
            <label><span>Тема</span><input name="subject" required minlength="3" maxlength="160" placeholder="Например: не загружается документ"></label>
            <label><span>Приоритет</span><select name="priority"><option value="normal">Обычный</option><option value="high">Высокий</option><option value="urgent">Срочный</option><option value="low">Низкий</option></select></label>
            <label><span>Что произошло</span><textarea name="message" required minlength="3" maxlength="5000" placeholder="Опишите шаги и результат. Пароли, ключи и токены не отправляйте."></textarea></label>
      ...
      return `<div class="access-layout">
        ...
        <section class="panel invite-panel">
          ...
          ${access.canManage ? `<form class="form" data-form="invite">
            ...
            <label><span>Контур</span><select name="accessMode" data-access-mode><option value="selected">Выбранные продукты</option><option value="all">Все продукты компании</option></select></label>
            <fieldset class="product-checks" data-product-checks><legend>Продукты</legend>${access.availableProducts.map((product) => `<label><input type="checkbox" name="productIds" value="${escapeText(product.id)}" ${product.id === this.state.data.currentProduct.id ? 'checked' : ''}><span>${escapeText(product.name)}</span></label>`).join('')}</fieldset>
            <p class="privacy-note">Срок ограничивает только активацию кода. После активации membership действует до suspend или revoke.</p>
      ...
      return `<article class="member">
        ...
        ${editing ? `<form class="member-editor form" data-form="member-access">
          ...
          <div class="form-grid">
            ...
            <label><span>Контур</span><select name="accessMode" data-access-mode><option value="selected" ${member.accessMode === 'selected' ? 'selected' : ''}>Выбранные продукты</option><option value="all" ${member.accessMode === 'all' ? 'selected' : ''}>Все продукты</option></select></label>
          </div>
          <fieldset class="product-checks ${member.accessMode === 'all' ? 'hidden' : ''}" data-product-checks><legend>Продукты</legend>${access.availableProducts.map((product) => `<label><input type="checkbox" name="productIds" value="${escapeText(product.id)}" ${member.productIds?.includes(product.id) ? 'checked' : ''}><span>${escapeText(product.name)}</span></label>`).join('')}</fieldset>
    ...
    bind() {
      ...
      this.shadowRoot.querySelectorAll('[data-action="switch-product"]').forEach((button) => {
        button.addEventListener('click', async () => {
          this.dispatchEvent(new CustomEvent('vertux-product-switch', { bubbles: true, composed: true }));
          if (typeof window.nexusProduct?.close === 'function') await window.nexusProduct.close('products');
        });
      });
      ...
      this.shadowRoot.querySelectorAll('[data-access-mode]').forEach((select) => {
        const sync = () => {
          const fieldset = select.closest('form')?.querySelector('[data-product-checks]');
          if (fieldset) fieldset.classList.toggle('hidden', select.value === 'all');
        };
      ...
      try {
        if (form.dataset.form === 'ticket') {
          ...
          await this.call('updateMemberAccess', {
            ...
            productIds: values.getAll('productIds'),
          });
          this.state.editMemberId = null;
